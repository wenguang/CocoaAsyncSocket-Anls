...
/**
  socket的数据被写入preBuffer，GCDAsyncReadPacket从preBuffer读取数据
  writePointer指向preBuffer可被写入数据的地址
  readPointer指向preBuffer未被读取过，又可被读取的地址
  preBuffer刚创建时，readPointer = writePointer = 0
  向preBuffer写入数据时，writePointer增加
  从preBuffer读取数据时，readPointer增加
  始终 readPointer <= writePointer，readPointer = writePointer时表示preBuffer没有数据可被读取了
 */
...
@interface GCDAsyncReadPacket : NSObject
{
  @public
	...
	NSUInteger startOffset;             //数据写入buffer的起始位置，目前除了初始化时对它有赋值，其他地方都没有
	NSUInteger bytesDone;               //已读入buffer的字节数
	NSUInteger maxLength;               //buffer的最大长度吗
	NSTimeInterval timeout;
	NSUInteger readLength;              //指定完整数据包的长度
	NSData *term;                       //指定完整数据包的分隔符
	BOOL bufferOwner;
	NSUInteger originalBufferLength;    //初始化参数传入的buffer原始长度
	long tag;
}
...
/**
  当我们不知道从socket读取多少字节长度时，会使用此方法。传入一个默认长度，看是否需要使用preBuffer，且返回一个合适的读取长度。
  注意：对于设置了readLength的包，是不需要对buffer扩容的，也就不需要使用preBuffer.
       对于 设置了分隔符的包、
            既不设置readLength也不设置分隔符的包、
            还有调用方传入buffer的包，需不需使用到preBuffer，
        要看情况而论。
 */
- (NSUInteger)optimalReadLengthWithDefault:(NSUInteger)defaultValue shouldPreBuffer:(BOOL *)shouldPreBufferPtr
{
	...
	if (readLength > 0)
	{
		...
		// 没有必要preBuffer，因为我们知道我们需要读取多少数据。
        // 即使buffer目前装不下这一数据量，那么最终还是要调整buffer大小，而不是使用preBuffer。
	...
	else
	{
        ...
        // 如果我们可以将所有的数据直接读入buffer，而不必首先调整它的大小，
        // 那么我们这样做，否则我们使用preBuffer。
...
/**
  对于没有指定分隔符的读取数据包，传入可读取的字节长度，返回可以buffer应该读取的长度。
  因为没有指定分隔符，就是不存在buffer扩容的可能，当然也就不需要用到preBuffer了。
  注意：返回长度不能超过readLength或maxLength。
       参数bytesAvailable可能来自preBuffer，也可能来自socket。
 */
- (NSUInteger)readLengthForNonTermWithHint:(NSUInteger)bytesAvailable
{
	...
	if (readLength > 0)
	{
        ...
        /*
         无需避免调整缓冲区大小。
         如果用户提供了自己的缓冲区，
         并告诉我们读取所有可用的数据，而不给我们一个maxLength，
         那么很明显我们的代码可能会在读取操作期间调整缓冲区大小。
      
         这个方法实际上并没有做任何调整大小。
         如果需要，调整大小将在其他地方发生。
         */
...
/**
  对于具有指定分隔符的读取数据包，传入一个socket上可读取的字节长度，
  计算是否要把数据读入preBuffer，否则直接读入buffer，返回应该从socket读取的字节长度。
  注意：参数bytesAvailable只能来自socket
 */
- (NSUInteger)readLengthForTermWithHint:(NSUInteger)bytesAvailable shouldPreBuffer:(BOOL *)shouldPreBufferPtr
{
    ...
    /*
     数据是否应读入buffer，还是首先读入preBuffer？
     
     人们会想到，首选的选择是哪个更快。那么哪一个更快？
     
     直接读入buffer需要：
     可能调整buffer大小（malloc / realloc）
     2.填充buffer
     3.搜索分隔符（memcmp）
     4.可能将溢出复制到prebuffer（malloc / realloc，memcpy）
     
     先读入preBuffer需要：
     可能调整preBuffer大小（malloc / realloc）
     2.填充preBuffer
     3.搜索分隔符（memcmp）
     4.将下溢复制到buffer（malloc / realloc，memcpy）
     5.从preBuffer中清除下溢（memmove）
     
     乍一看，读入preBuffer较慢些，因为它多了一步内存移动的操作。
     ...
     而buffer通常可能需要realloc。
     如果我们是buffer所有者，这一点尤其如此。
     此外，如果我们不断重新分配buffer的内存，
     然后将溢出部分移动到preBuffer中，
     那么我们会为每一次读取分隔符而分配内存。
...
/**
  对于具有指定分隔符的读取数据包，传入一个preBuffer，找到preBuffer是否出现了分隔符，返回应该从preBuffer读取数据的长度。
  若preBuffer中没有出现分隔会，也要返回一个适当的长度，它是preBuffer可读取的长度且不超过buffer的最大长度限制。
  注意：可能从preBuffer的数据起始位置不一定是0，因为该preBuffer很可能已被读取了部分数据。
 */
- (NSUInteger)readLengthForTermWithPreBuffer:(GCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr
{
	NSAssert(term != nil, @"This method does not apply to non-term reads");
    ...
    /*
     有种特殊情况：
     完成的终止符不在数据包buffer中，但可能它的部分在数据包buffer中，
     所以要从含有部分终止符的数据包buffer开始找，直到找到preBuffer中终止符的起始位置，
     终止符的长度为termLength，那就应该从buffer的倒数第termLength-1个字节开始找。
     要实现以上，就先把buffer的数据和preBuffer数据并在暂时的内存中，然后逐个移位比较。
    ...
    // 特殊情况：已读入buffer的长度 + preBuffer的可读取长度 之和 都小于分隔符长度
	if ((bytesDone + preBufferLength) < termLength)
	{
		return preBufferLength;
	}
	
    // 可从preBuffer读取的长度不能超过 buffer的设置的最大长度 - buffer已读入的长度
	NSUInteger maxPreBufferLength;
	if (maxLength > 0) {
		maxPreBufferLength = MIN(preBufferLength, (maxLength - bytesDone));
		
		// maxLength一定大于termLength
	}
	else {
		maxPreBufferLength = preBufferLength;
	}
	
    // seq是用来移动比较的字节序，按以上说的特殊情况，它有可能由 buffer部分字节 + preBuffer部分字节 拼接而成
	uint8_t seq[termLength];
	const void *termBuf = [term bytes];
	
    // bufLen表示：buffer中占有分隔符的长度，它不总是termLength - 1，因为有可能 bytesDone < termLength - 1
    // buf表示：指向buffer在移位比较中的起始地址，该指针随移位比较逐步加1
	NSUInteger bufLen = MIN(bytesDone, (termLength - 1));
	uint8_t *buf = (uint8_t *)[buffer mutableBytes] + startOffset + bytesDone - bufLen;
	
    // preLen表示：preBuffer中占有分隔符的长度
    // pre表示：指向preBuffer在移位比较中的起始地址，当移位比较全部在perBuffer中进行时，该指针随移位比较逐步加1
	...
	for (i = 0; i < loopCount; i++)
	{
		...
		else
		{
			...
			{
                // preOffset表示：当找到分隔符时，pre指针移动了多少字节
...
/**
  对设置了分隔符的包，找到buffer溢出的部分，以便可能拷贝到preBuffer中
  因为只是从socket读了一块数据，不确定这一块数据中是否包含了分隔符，所在就用这个方法查找
 
  searchForTermAfterPreBuffering 这个方法倒是有点不好理解
 
  该方法只有以上情况都具备才可能被调用
  1、设置了分隔符的包
  2、刚从socket读取数据写入包的buffer
  3、数据刚被写入buffer，但还未更新bytesDone
 
  numBytes：刚写入buffer的数据长度
 
  返回：溢出的字节数，-1：未找到term，0：正好term就在buffer尾部
**/
- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes
{
	NSAssert(term != nil, @"This method does not apply to non-term reads");
	...
	// i表示：从buffer查找term的起始位置
    // 在未写入numBytes字节前，buffer最大可能包含term的前termLength - 1字节
...
#pragma mark - 断点不能进globalConcurrentQueue
...
- (void)doReadData
{
    ...
    if ([preBuffer availableBytes] > 0)
    {
        ...
        // 用GCDAsyncReadPacket的计算方法，得到从preBuffer读入数据的合适长度
        
        if (currentRead->term != nil)
        {
            // 从preBuffer找分隔符，且返回合适长度
        ...
        // 保证GCDAsyncReadPacket的buffer足够装下bytesToCopy长度的数据，有必要则对buffer扩容
        
        [currentRead ensureCapacityForAdditionalDataOfLength:bytesToCopy];
        
        
        // 指针指向buffer接受拷贝数据的起始地址
        
        uint8_t *buffer = (uint8_t *)[currentRead->buffer mutableBytes] + currentRead->startOffset +
        currentRead->bytesDone;
        
        // 从preBuffer拷贝数据到currentRead的buffer
        
        memcpy(buffer, [preBuffer readBuffer], bytesToCopy);
        
        // 让preBuffer的readPointer指针地址加bytesToCopy位
        ...
        // 检查经过数据拷贝后，currentRead数据包是否已完整
        
        if (currentRead->readLength > 0)
        {
            // 对于类型2，读入buffer的数据量 == 包指定长度 则为完整
            
            done = (currentRead->bytesDone == currentRead->readLength);
        }
        else if (currentRead->term != nil)
        {
            // 对应类型3，能用readLengthForTermWithPreBuffer:found:方法找到preBuffer的分隔符即为完整
            //
        ...
        {
            // 对于类型3，读入buffer的数据量 == buffer的最大限制 则为完整
    ...
    if (!done && !error && !socketEOF && hasBytesAvailable)
    {
        ...
        // 确定preBuffer或currentRead写入的起始地址
        // 有需要的话，对currentRead或preBuffer进行扩容
        ...
        else
        {
            ...
            {
                // 读到的数据长度比请求的数据长度要短
                // 上面我们是传入socketFDBytesAvailable让GCDAsyncReadPacket计算后得的bytesToRead
                // bytesToRead最大也就是等于socketFDBytesAvailable了，socketFDBytesAvailable又是从readSource得到的socket的可读数据
                // 而read函数返回的结果又小于bytesToRead，出现这种说明：
                ...
                // 这意味着socketFDBytesAvailable由于时序有点偏离，因为当readSource事件触发时，我们从套接字读取。
                
#pragma mark - 这意味着socketFDBytesAvailable由于时序有点偏离，因为当readSource事件触发时，我们从套接字读取
                
                socketFDBytesAvailable = 0;
            }
            else
            {
#pragma mark - 还有这种路径~~ 奇怪
        ...
        if (bytesRead > 0)
        {
            ...
            if (currentRead->readLength > 0) // 类型2：读入指定长度的数据，该类型不需要用到preBuffer
            {
            ...
            else if (currentRead->term != nil)  // 类型3：读入到分隔符为止的数据
            {
                // 对应socket数据先写入preBuffer的情况
                // 把刚刚从socket读入preBuffer的数据，拷贝到currentRead中
                if (readIntoPreBuffer)
                {
                ...
                {
                    // 设置了分隔符的包，没用到preBuffer，用read函数直接从socket读到currentRead的buffer中
                    // 因为只是从socket读了一块数据，不确定这一块数据中是否包含了分隔符
                    // 所以找到currentRead的buffer溢出的字节数，以便拷贝到preBuffer中
                    ...
                    else if (overflow > 0)
                    {
                        ...
                        // 虽然已经向currentRead->buffer写入bytesRead字节数据，
                        // 但此时，buffer指针仍指向read函数之前的地址
#pragma mark - 这里buffer很好地体现了指针和地址的关系，反复品，这是重要的基础啊~~~
	...
	switch(type)
	{
		case kCFStreamEventHasBytesAvailable:
		{
			dispatch_async(asyncSocket->socketQueue, ^{ @autoreleasepool {
				...
				if ((asyncSocket->flag & kStartingReadTLS) && (asyncSocket->flags & kStartingWriteTLS))
				{
